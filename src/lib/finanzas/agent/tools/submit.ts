// enviar_a_bandeja: el asistente manda las líneas del estado de cuenta ya
// clasificadas. Cada una cae en la Bandeja (fin_inbox) como "pendiente". NADA
// se registra en compras, gastos ni pagos hasta que el dueño apruebe en la app.
//
// Es la ÚNICA herramienta que escribe, y solo escribe en fin_inbox.
//
...
import { addDays, diffDays, uuid, ymd } from '../params';
...
// ---------------------------------------------------------------------------
const Line = z.object({
  line_no: z.number().int().min(0).optional().describe('Número de línea en el estado de cuenta.'),
  raw_text: z.string().trim().min(1).max(500).describe('La línea tal cual aparece en el banco.'),
  kind: z.enum(['compra', 'gasto', 'abono']),
  date: ymd.describe('Fecha del movimiento AAAA-MM-DD.'),
  amount: z.number().positive().describe('Monto positivo en la moneda indicada.'),
  currency: z.enum(['USD', 'VES']).default('USD'),
  bcv_rate: z.number().positive().optional().describe('Solo VES. Si se omite se usa la guardada para ese día.'),
  account_id: uuid.optional().describe('Cuenta o tarjeta de donde salió el dinero (de obtener_contexto). Solo forma de pago.'),
  supplier_id: uuid.optional().describe('Proveedor existente (de obtener_contexto).'),
  supplier_name: z.string().trim().max(120).optional().describe('Si el proveedor no existe: nombre limpio. Se crea al aprobar.'),
  category_id: uuid.optional().describe('Categoría (de obtener_contexto).'),
  expense_id: uuid.optional().describe('Solo abono: la compra o gasto que se abona (de listar_compras_gastos).'),
  description: z.string().trim().max(300).optional(),
  due_date: ymd.optional().describe('Vencimiento, solo si no está pagada.'),
  paid: z.boolean().default(true).describe('Compra/gasto ya pagado. Del estado de cuenta, siempre true.'),
  is_personal: z.boolean().default(false),
  reference: z.string().trim().max(100).optional().describe('Número de referencia o confirmación del banco, si lo hay.'),
  note: z.string().trim().max(500).optional().describe('Por qué lo clasificaste así (lo ve el dueño).'),
});
...
// Herramienta
// ---------------------------------------------------------------------------
export const submitTool = defineTool({
...
async function submitLines(
  ...
  lines.forEach((line, idx) => {
    ...
    if (line.account_id && !validAccounts.has(line.account_id)) return fail('account_id no existe. Usa los de obtener_contexto.');
    if (line.category_id && !validCategories.has(line.category_id)) return fail('category_id no existe. Usa los de obtener_contexto.');
    if (line.supplier_id && !suppliersById.has(line.supplier_id)) return fail('supplier_id no existe. Usa los de obtener_contexto.');

    // Tasa: la del asistente, o la guardada ese día. Sin tasa no se puede convertir.
    ...
    if (line.kind === 'abono') {
      paid = true;
      if (!line.expense_id) return fail('Un abono necesita expense_id (búscalo con listar_compras_gastos, status=abiertas).');
