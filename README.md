...
* **Qué mueve el saldo de una cuenta y qué no** (`db/finanzas_05_suscripciones_y_abonos.sql`). Las **compras y los fletes sí** lo mueven: ahí importa cuánto queda en la tarjeta. Los **gastos operativos no**: la sección de Gastos es para llevar la cuenta de en qué se va el dinero, así que guarda con qué cuenta se pagó como información, pero no toca el saldo. Y los **abonos y cargos manuales** (`fin_account_movements`) sí, que es como el dueño baja la deuda de una tarjeta — antes la única forma era editar el saldo inicial, y eso borraba el historial.
