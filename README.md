...
* **`profiles`**: Extiende `auth.users`. Almacena el `full_name`, el `role`, la atadura a la sucursal obligatoria (`assigned_store_id`) y `can_restock_all` (permiso especial de "reponedor": un cajero con esta bandera puede **subir** stock en **todas** las tiendas, no solo la suya). El owner lo activa/desactiva en `/users`.
...
* **`products` (Catálogo Global):** Almacena `sku_barcode`, `name`, `category`, `price` y `owner_store_id`. El producto sigue siendo **vendible en cualquier tienda desde el POS** (catálogo global), pero `owner_store_id` define la **tienda dueña**: el módulo de Inventario muestra **solo** los productos de la tienda activa. Las categorías son `juguetes`, `ropa`, `zapato`, `perfume`. El `sku_barcode` autogenerado usa el prefijo de la **tienda** dueña (`JUG-`/`ROP-`), no de la categoría; si se escanea un código, se respeta tal cual.
* **`store_stock` (Inventario Local):** Nueva tabla que maneja el stock físico por tienda `(product_id, store_id)`. **Permite stock negativo**: si se vende un producto sin existencias, el stock de esa tienda queda en negativo (p. ej. -1) para reflejar la sobreventa (se removió el `CHECK (stock >= 0)`). **Incluye un Trigger automático** que inicializa el stock en 0 en todas las sucursales cuando nace un nuevo producto. En el módulo de Inventario, el **cajero solo puede aumentar** el stock manualmente (nunca reducirlo); el `owner` puede subirlo o bajarlo. El módulo tiene además un **filtro de solo-vista** (ambos roles) para mirar el inventario de otra tienda, sin poder gestionarla (cada usuario opera únicamente sobre su tienda asignada/activa).
* **`sales`**: Cabecera de facturación. Vinculada obligatoriamente al cajero (`cashier_id`), cliente (`customer_id`) y sucursal (`store_id`). Guarda el `total_amount`, el `redemption_discount_usd` (descuento por canje aplicado, para auditoría en el historial), método de pago y la tasa BCV exacta.
