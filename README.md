...
* **Qué mueve el saldo de una cuenta y qué no** (`db/finanzas_05_suscripciones_y_abonos.sql`). Solo las **compras a proveedores sí** lo mueven: ahí importa cuánto queda en la tarjeta. Los **gastos operativos y los fletes de las cajas no** (`db/finanzas_06_fletes_no_restan.sql`): guardan con qué cuenta se pagaron como información, pero no tocan el saldo. La vista filtra `kind = 'compra'` y no "todo menos gastos", a propósito: un tipo de egreso nuevo, por defecto, no mueve saldos. Y los **abonos y cargos manuales** (`fin_account_movements`) sí, que es como el dueño baja la deuda de una tarjeta — antes la única forma era editar el saldo inicial, y eso borraba el historial.
