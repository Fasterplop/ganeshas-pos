...
* **Tablas `fin_*` (Módulo de Finanzas, NUEVO):** nueve tablas aisladas del POS (`fin_suppliers`, `fin_categories`, `fin_accounts`, `fin_shipments`, `fin_shipment_items`, `fin_expenses`, `fin_purchase_lines`, `fin_payments`, `fin_budgets`) más tres vistas de reporte. **Ninguna tabla del POS tiene FK hacia ellas**, así que el módulo se puede revertir entero (`db/finanzas_99_rollback.sql`) sin tocar ventas, productos ni stock. Todas nacen con RLS y las **cuatro** políticas `owner-only` en el mismo archivo (`db/finanzas_01_schema.sql`), por la trampa de `ensure_rls` que se documenta arriba. Proveedores, categorías, cuentas y tarjetas son **globales del negocio** (sin `store_id`); compras, gastos, cajas y presupuestos sí distinguen sucursal. Ver módulo 10.
...
**Estado de la entrega.** Construido: el shell con las ocho pestañas y el guard, **proveedores** con estado de cuenta, **control de cajas** (recepción incompleta, foto de la guía, costo de envío y export), **cuentas y tarjetas** con saldo inicial y saldos derivados, y **compras y cuentas por pagar** (abonos parciales, pago mixto, aviso de duplicado, detalle opcional, foto del recibo y enlace del contenido de las cajas a su compra). Pendientes, en este orden: gastos operativos con presupuesto y calendario de pagos → dashboard financiero y las 8 exportaciones → estado financiero personal. El esquema completo entra de una sola vez con `db/finanzas_01_schema.sql`, así que ninguna fase siguiente obliga a migrar de nuevo ni a recargar datos: por ejemplo, el contenido que hoy se escribe a mano en una caja solo recibirá su `expense_id` cuando llegue el registro de compras.
