...
**Estado de la entrega.** Construido: el shell con las ocho pestañas y el guard, **proveedores**, **control de cajas** (recepción incompleta, foto de la guía y export a Excel), **cuentas y tarjetas** con saldo inicial y saldos derivados, y el **costo de envío** por caja (que ya ejercita la maquinaria de egreso + pago). Pendientes, en este orden: compras y cuentas por pagar → gastos, presupuesto y calendario → dashboard financiero y las 8 exportaciones → estado financiero personal. El esquema completo entra de una sola vez con `db/finanzas_01_schema.sql`, así que ninguna fase siguiente obliga a migrar de nuevo ni a recargar datos: por ejemplo, el contenido que hoy se escribe a mano en una caja solo recibirá su `expense_id` cuando llegue el registro de compras.
